...
def test_create_read_update_delete_todo(todos_client, users_client, auth_headers, created_user):
    ...
    get_response = todos_client.get_todo(todo["id"])
    ...
    list_by_query = todos_client.list_todos(params={"user_id": created_user["id"]})
    assert_status(list_by_query, 200)
    assert any(item["id"] == todo["id"] for item in list_by_query.json())

    list_by_parent = users_client.list_todos_for_user(created_user["id"])
    ...
    get_deleted = todos_client.get_todo(todo["id"])
