...
def test_create_read_update_delete_post(posts_client, users_client, auth_headers, created_user):
    ...
    get_response = posts_client.get_post(post["id"])
    ...
    list_by_query = posts_client.list_posts(params={"user_id": created_user["id"]})
    assert_status(list_by_query, 200)
    assert any(item["id"] == post["id"] for item in list_by_query.json())

    list_by_parent = users_client.list_posts_for_user(created_user["id"])
    ...
    get_deleted = posts_client.get_post(post["id"])
